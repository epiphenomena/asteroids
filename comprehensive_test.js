...
console.log('Comprehensive game test script loaded');
...
function testCoreFunctions() {
    ...
    const coreFunctions = ['init', 'resetGame', 'gameLoop', 'update', 'render'];
...
function testObjectCreation() {
    ...
    if (typeof createPowerup !== 'undefined') {
        console.log('✓ Powerup creation function exists');
        ...
        setTimeout(() => {
            if (powerups.length > initialCount) {
                ...
                console.log('! Powerup creation may have issues');
            }
        }, 500);
    } else {
        console.log('✗ Powerup creation function does not exist');
    }
...
function testRendering() {
    ...
    const drawFunctions = [
        ...
        'drawParticles', 'drawRadarIndicators'
    ];
